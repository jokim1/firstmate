...
Every captain-facing message, including a one-line acknowledgement, must open with the message tag whose exact format is owned by `data/captain.md`; the tag is authorized even when it names a model or effort level, but this exception licenses only the tag and not the prose that follows it.
