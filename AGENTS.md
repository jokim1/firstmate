...
When a steer answers an open keyed decision or blocker, pass `fm-send`'s `--resolve-key` so confirmed delivery closes that decision record identically for local and remote workers while an unconfirmed delivery leaves it open (contract: `bin/fm-send.sh` header).
...
Send the same worker one exact decision naming the decision key, step, action, affected finding IDs, instructions where needed, and exact response command, passing `--resolve-key` so confirmed delivery closes the worker's open decision record.
