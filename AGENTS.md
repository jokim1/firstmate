...
`playbot` is an experimental, never-default, worktree-owning backend selected only by explicit per-task authority; live dispatch stays phase-gated until Phase 1/2 native gates pass, and it refuses secondmates and ship modes other than local-only (v1).
