...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then tmux; tmux is the verified reference backend (docs/tmux-backend.md), herdr has its own required CI lane (docs/herdr-backend.md), while zellij, orca, cmux, and playbot remain experimental with no dedicated real-backend CI lane (docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md, docs/playbot-lanes.md when present) - herdr and cmux can also be selected by runtime auto-detection; zellij, orca, and playbot never are (always explicit); live playbot dispatch requires the adapter's native-readiness verdict for the installed release; codex-app is not accepted; see docs/codex-app-backend.md; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  standing-authority.md  main-authoritative register of standing captain grants propagated read-only to secondmate homes and consulted under section 9; entries point at each grant's authoritative owner and never restate it; LOCAL, gitignored, owned by secondmate-provisioning
...
state/               runtime records and signals; gitignored
  ...
  .focus.json        primary focus lifecycle snapshot (active + suspended stack); single owner bin/fm-focus.sh
...
`playbot` is an experimental, never-default, worktree-owning backend selected only by explicit per-task authority; live dispatch requires the adapter's native-readiness verdict for the installed release, and it refuses secondmates and ship modes other than local-only (v1).
There is no automatic captain phrase trigger for Playbot; setup and doctor workflow live in the playbot-lanes skill when installed.
Windows is unsupported for the native Playbot lane until a separate live proof.
...
After successful teardown, record completion and retain only the configured recent Done history.
...
The captain-facing message tag owned by `data/captain.md` is authorized even when it names a model or effort level, but this exception licenses only the tag and not the prose that follows it.
...
Before asking the captain for permission, or before overriding, dropping, or declining a recorded obligation, consult this home's standing authority register at `data/standing-authority.md` for an existing grant covering that exact object; when one applies, act on it and report the action instead of asking or overriding.
The register cites each grant's authoritative owner rather than restating rulings; when the register is absent, consult its owners directly in `data/captain.md`, `data/captain-shared.md`, `data/projects.md`, `data/patch-queue/README.md`, and the `ask-user-authority` skill instead of defaulting to asking.
...
Re-evaluate queued work on every `refill:` and `heartbeat:` wake - a capacity-freeing teardown or status transition enqueues that refill - dispatching items only when dependencies and time gates have cleared.
...
- `patch-queue-prune` - load at every `lila-main` upstream rebase, when the captain asks for a patch-stack prune, refactor, or simplification audit, and when `data/patch-queue/README.md`'s rebase records show roughly six weeks without a rebase.
