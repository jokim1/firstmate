...
# CLAUDECODE=1 into a codex pane). Print an `env -u ...` prefix that drops every
...
foreign_marker_scrub_prefix() {
  ...
  case "$harness" in
    claude)
      ...
      printf '%s' 'env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u PI_CODING_AGENT -u FM_PI_HARNESS -u GROK_AGENT'
      ;;
    pi|pi-signed)
      # Keep PI_CODING_AGENT and FM_PI_HARNESS (the latter is re-set on the
      # launch line for the selected Pi identity).
      printf '%s' 'env -u CLAUDECODE -u GROK_AGENT -u CURSOR_AGENT -u CURSOR_INVOKED_AS'
      ;;
    grok)
      # Keep GROK_AGENT; scrub every non-grok verified marker.
      printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u FM_PI_HARNESS -u CURSOR_AGENT -u CURSOR_INVOKED_AS'
      ;;
    cursor)
      # Keep CURSOR_AGENT; the cursor launch template additionally unsets the
      # legacy CURSOR_INVOKED_AS alias marker itself.
      printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u FM_PI_HARNESS -u GROK_AGENT'
      ;;
    *)
      printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u CURSOR_AGENT -u CURSOR_INVOKED_AS'
      ;;
  esac
}
