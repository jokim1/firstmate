...
# (codex, opencode, kimi, muse, agy) and unknown/raw targets drop all of them.
...
foreign_marker_scrub_prefix() {
  ...
  case "$harness" in
    ...
    omp)
      ...
      ;;
    *)
      # Markerless adapters (codex, opencode, kimi, muse, agy) and unknown/raw
      # targets carry no verified marker of their own, so scrub all of them.
...
launch_template() {
  ...
  # inherited marker. Foreign primary-marker scrub is now owned centrally by
  # foreign_marker_scrub_prefix and applied to every launch below, so the muse
  # template no longer clears markers inline; its central env prefix still
  # carries the XDG_*/MUSE_* assignments the template sets.
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # This foreign-slot check runs before the ownership claim below so a slot we
  # are about to reject never has this task's claim written into it.
