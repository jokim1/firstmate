    ...
    for c in "$STATE"/*.check.sh; do
      ...
      if [ -n "$out" ]; then
        ...
        if [ "$is_pr_poll" -eq 1 ] && [ "$out" = merged ]; then
          # Capacity free after a first merged-PR notification: one advisory
          # refill wake. Duplicate already-notified merges are absorbed above
          # and do not enqueue again.
