...
# Resolved records leave the hot state/pending-replies/ walk once their
# escalation lifecycle is closed, or - when close cannot complete (blank
# parent_status, unwritable status, permanent close failure) - via quarantine to
# state/pending-replies-stuck/ with a durable receipt. That keeps open
# status-fold decisions intact while the hot poll cannot re-accumulate answered
# files until a single iteration starves the liveness beacon. Unresolved records
# are never silently expired.
#
...
fm_pending_reply_tick() {  # <state-dir> [beat-path] [beat-interval] [owner-pid] [watch-path] [owner-home]
  ...
  for rec in "$dir"/*; do
    ...
    case "$phase" in
      recovery_failed|recovery_unknown)
        fm_pending_reply_tick_one "$state" "$corr" unknown "" || true
        # A late reply resolving at the recovery-failure boundary stays readable
        # as resolved this poll: the resolution must be visible so it is not
        # confused with a recovery-delivery escalation. The top-of-loop retire
        # reaps it on a subsequent poll, keeping later walks cheap.
        continue
        ;;
    esac
    case "$phase" in
