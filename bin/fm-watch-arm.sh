...
# a fresh liveness beacon (state/.last-watcher-beat within FM_GUARD_GRACE, the
...
watch_output_has_wake() {
  local out=$1
  grep -Eq '^(signal:|stale:|check:|heartbeat($|:)|refill($|:))' "$out" 2>/dev/null
}

watch_output_reason_type() {
  local out=$1 line
  line=$(grep -E '^(signal:|stale:|check:|heartbeat($|:)|refill($|:))' "$out" 2>/dev/null | head -1 || true)
  case "$line" in
    ...
    refill*) printf 'actionable-refill' ;;
    *) printf 'none' ;;
  esac
}
