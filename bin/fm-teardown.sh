...
# the retired home. Before the home is removed, teardown destroys every treehouse
# pool slot whose git common dir lies under that home (destroy_home_bound_pool_slots),
# while the home's project clones still exist: treehouse keys pools by origin URL
# under a shared root, so slots created from the home's clones can live in pools
# other homes use, and deleting the clones first would orphan those slots and
# poison the pool. A slot treehouse will not destroy bare (in use, or unlanded
# without --force) refuses the retirement loudly. Removing a leased home releases
# its durable treehouse lease so the pool slot is freed,
...
# Drop any parked steers so a relaunch or successor cannot inherit ghost
# follow-ups from this incarnation (bin/fm-send-followup-lib.sh).
fm_send_followup_invalidate "$STATE" "$ID" || true
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.meta" \
  ...
  "$STATE/$ID.followup-gen"
rm -rf "$STATE/$ID.followup-queue" "$STATE/$ID.followup-queue.lock"
