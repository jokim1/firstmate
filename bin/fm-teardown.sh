...
# return attempt (including lock retries). Uncommitted changes are never landed.
...
# work, kills child runtime endpoints, and removes the retired home. Before the
# home is
# removed, teardown destroys every treehouse pool slot whose git common dir lies
# under that home (destroy_home_bound_pool_slots), while the home's project
# clones still exist: treehouse keys pools by origin URL under a shared root, so
# slots created from the home's clones can live in pools other homes use, and
# deleting the clones first would orphan those slots and poison the pool. A slot
# treehouse will not destroy bare (in use, or unlanded without --force) refuses
# the retirement loudly. Removing a leased home releases its durable treehouse
# lease so the pool slot is freed,
...
teardown_is_tmux_husk() {  # <meta-file> <task-id>
  ...
  foreign=$(grep -cE '^(herdr_|zellij_|orca_|cmux_|terminal=)' "$meta" 2>/dev/null || true)
  [ "$foreign" = 0 ] || return 1
...
ship_land_proven() {
  ...
  # T: only when no pr= (unconfirmed with pr= must not use T — R2-R1). A recorded
  # value that does not parse as a PR URL at all carries no forge state to
  # overlay, so it is treated as no pr= here; the close-marker write still
  # refuses to record such a value, which is where a malformed pr= belongs.
  if [ -z "${PR_URL:-}" ] || ! fm_pr_url_parse "$PR_URL" 2>/dev/null; then
  ...
  # CLOSED or unconfirmed with pr=: ancestry (A) already failed above. A
  # branch-deleted squash merge leaves classify_recorded_pr_state unable to
  # confirm the state by branch, but the head-based merged check still proves
  # the work landed - the same signal upstream's work_is_landed trusts. Accept
...
validate_worktree_teardown_safety() {
  ...
  # Exception: a local-only task lands on local main with no remote or PR to
  # strand, so a gone worktree leaves nothing to inspect or discard - complete
  # the teardown as upstream does rather than wedging a task no restore can help.
  if [ ! -d "${WT:-}" ]; then
...
# teardown_owns_worktree (upstream #4243): a pool slot reassigned to another
# task is no longer ours to inspect, so skip the land/dirty validation the same
# way upstream's guard did before this land-gate restructure; the reassigned
# slot's destructive cleanup is skipped by the matching branch below.
if teardown_require_land_gates && teardown_owns_worktree; then
  case "$KIND" in
    secondmate|scout) ;;
    *)
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  # Pool slot reassigned to another task (upstream #4243): leave its worktree
  # untouched; only this task's own endpoint, records, and backlog are cleaned.
  :
...
if [ "$HERDR_PRESENTATION_RETIRE_CANDIDATE" = 1 ]; then
  ...
  # A husk or endpoint-less record has nothing to close, so there is no close to
  # refuse on; a real endpoint keeps upstream #4510's hard close-or-refuse.
...
if [ "$KIND" = secondmate ]; then
  ...
  if ! destroy_home_bound_pool_slots "$HOME_PATH_REAL"; then
    handoff_wake_retire_stage_restore \
      || echo "error: receiver wake restoration failed; recovery state remains at $HANDOFF_WAKE_RETIRE_STAGE" >&2
    exit 1
  fi
