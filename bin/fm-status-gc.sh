...
# too. A per-task record that lives one level down is exactly as disqualifying
# as a top-level one.
...
fm_gc_references_task() {  # <record-name> <id>
  local name=$1 id=$2
  # `/` is a delimiter as much as `.`, `-`, and `_`: a nested name can carry the
  # id immediately after a slash, and leaving `/` out lets that shape fail OPEN.
  case "$name" in
