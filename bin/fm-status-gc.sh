...
#   4. No per-task temp root /tmp/fm-<id>. Spawn creates it BEFORE it creates any
#      endpoint (bin/fm-spawn.sh), so it is the earliest durable trace that a
#      spawn ran for this id on any backend, and its presence means cleanup never
#      finished.
...
# here. Gate 4 is the substitute, and it is only as good as that ordering: every
# ordinary backend creates its window or task before the worktree settles and
# long before the meta is published, so a spawn that dies in between leaves an
# endpoint behind with no record naming it. The temp root is created ahead of all
# of that precisely so this gate covers that window too. The residual is an
# endpoint created outside firstmate's own spawn path, which no meta-less check
# could see either.
#
