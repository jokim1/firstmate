...
The main firstmate's answer normally writes that closing line after confirmed delivery; when a blocker or wait clears WITHOUT an answer from the main firstmate, append \`resolved: {how it cleared}\` yourself (keyed with \`[key=<slug>]\` if you opened it with one) as your domain resumes.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   Firstmate's reply normally writes that closing line after confirmed delivery; when a blocker or wait clears WITHOUT a firstmate reply, append \`resolved: {how it cleared}\` yourself (same \`[key=<slug>]\` if you opened it with one) as you resume.
