...
if [ "$LOOP_COUNT" -ge "$LOOP_CEILING" ]; then
  ...
  fm_supervision_cycle_needed "$STATE" "$GRACE" || exit 0
...
if ! fm_supervision_cycle_needed "$STATE" "$GRACE"; then
  budget_reset_if_ours
  exit 0
fi
...
# The need may have vanished while parked - the fleet was torn down, the queue
# drained, or Relay was opted out. Nothing left to hold a cycle for, so end
# the turn quietly.
if ! fm_supervision_cycle_needed "$STATE" "$GRACE"; then
  budget_reset_if_ours
  exit 0
