...
# The blocking job must sleep WELL past its 3s execution window so the worker's
# timeout kill fires deterministically (exit 124). fm-timeout-job.sh sleeps
# exactly the 3s window, so under CI scheduling jitter its own sleep can return
# on the same tick as the timeout and publish a normal exit 0 instead - a
# boundary race, not a real behavior change. A 30s sleep removes the race; the
# job is killed at the window and never writes its sink.
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  fm-delay-job.sh 30 "$TMP_ROOT/blocking-window-sink" < /dev/null > /dev/null
FIRST_JOB_ID=$FM_REMOTE_JOB_ID
FIRST_JOB_DIR="$STATE_ROOT/jobs/$FIRST_JOB_ID"
for _ in $(seq 1 100); do
  [ "$(fm_remote_job_read_state "$FIRST_JOB_DIR" 2>/dev/null || true)" = running ] && break
