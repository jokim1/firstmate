...
# live tmux/treehouse/fleet state is touched. The fixture intentionally has no
# worktree, so its teardown calls use --force to bypass landing proof and isolate
# tasktmp cleanup behavior.
make_fake_root() {
...
SH
  ...
  # Shared task-record retirement owns busy cleanup and invokes its writer.
  ln -s "$ROOT/bin/fm-task-records-retire-lib.sh" "$fake/bin/fm-task-records-retire-lib.sh"
  ln -s "$ROOT/bin/fm-busy-lib.sh" "$fake/bin/fm-busy-lib.sh"
  ln -s "$ROOT/bin/fm-busy-event.sh" "$fake/bin/fm-busy-event.sh"
