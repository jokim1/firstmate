...
test_drain_dedupes_refill_by_kind() {
  ...
  # Drain is durable until post-handling ack: a second drain must re-present the
  # same collapsed set rather than empty (matches heartbeat/signal drain contract).
  leftover=$(FM_STATE_OVERRIDE="$state" "$DRAIN" | awk -F '\t' 'NF == 5 { count++ } END { print count + 0 }')
  [ "$leftover" -eq 3 ] || fail "expected durable replay of 3 collapsed records after drain, got $leftover"
