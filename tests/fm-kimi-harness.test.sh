...
test_kimi_launch_then_send_is_verified() {
  ...
  [ "$launch" = "env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI '$FAKEBIN_DIR/kimi' --model 'kimi-code/k3' --auto" ] \
    || fail "kimi launch did not use the absolute binary, model, and --auto only (with foreign-marker scrub): $launch"
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  launch=$(cat "$CASE_DIR/launch.log")
  [ "$launch" = "env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI '$fallback' --auto" ] \
    || fail "Kimi fallback did not expand HOME into an absolute executable (with foreign-marker scrub): $launch"
...
test_kimi_status_footers_keep_bordered_composer_readable() {
  ...
  pending_042=${pending_042/ thinking / thinking: /}
  empty_042=${empty/Version:   0.41.0/Version:   0.42.0}
  empty_042=${empty_042/ thinking / thinking: /}
  ...
  leftbar_042=${leftbar/ thinking / thinking: /}
