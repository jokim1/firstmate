...
seed_repro_commitment() {   # <home> <obligation> <request> <work-home> <work-id>
  ...
  jq -n --arg r "$request" \
    '{request_id:$r, platform:"discord",
      ...
      followup_expires_at:"2099-08-28T01:12:00Z",
      reservation_expires_at:"2099-08-28T01:12:00Z"}' > "$home/request.json"
...
test_rechain_refuses_unclaimed_existing_destination() {
  ...
  tasks_in "$home" public-followup add public-final-existing-b \
    ...
    --expires-at 2099-08-28T01:12:00Z >/dev/null || fail "could not seed destination collision"
...
test_expiry_escalation_uses_now_override() {
  ...
  exp=$(date -u -j -f '%Y-%m-%dT%H:%M:%SZ' '2099-08-28T01:12:00Z' +%s 2>/dev/null) \
    || exp=$(date -u -d '2099-08-28T01:12:00Z' +%s)
