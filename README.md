...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca when explicitly selected); interrupted primary focus remains durably resumable, and the next session reconciles confirmed-dead secondmate agents and carries on.
