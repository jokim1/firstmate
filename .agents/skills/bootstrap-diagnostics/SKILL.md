...
- `SECONDMATE_SYNC: secondmate <id>: skipped: <reason>` - secondmate convergence left a live home on its existing checkout because the home was dirty, diverged, unsafe, on the wrong branch, missing its placement-specific target commit, unreachable, or otherwise not fast-forwardable, or because inherited local-material propagation failed; bootstrap continued, but inspect the reason because the secondmate's tracked instructions, inherited settings, or shared data may be stale after a primary update.
- `SECONDMATE_LIVENESS: relaunch gate unavailable under <dir>; relaunching <id> ungated` - the parallel sweep could not create its private relaunch gate, so that worker continued without serialization and may contend with another relaunch for the per-home task-set lock.
  Inspect why the named scratch directory disappeared or refused a subdirectory, and rerun session start after correcting it so any separately reported relaunch failure is retried.
