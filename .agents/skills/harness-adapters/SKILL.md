...
| Environment marker | None. Detection is process ancestry on the anchored prefix `muse-bin-*`. Every launch (every harness) clears foreign primary markers via `foreign_marker_scrub_prefix` in `bin/fm-spawn.sh` so marker precedence cannot override that ancestry. `MUSE_CURRENT_SESSION_LOG` is a session-log PATH rather than an identity, and its export to tool subprocesses is unverified. |
