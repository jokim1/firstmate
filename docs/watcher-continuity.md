...
The hook fires on every Stop, and an eligible primary with standing supervision need or any unread wake-queue row admits one home-scoped owner that foregrounds `bin/fm-watch-arm.sh` inside the hook-owned process tree.
An idle empty-queue home stays inert; an idle home with only advisory queue rows still holds one cycle so those rows can rewake instead of aging the beacon while the parent stall predicate alarms.
...
Kimi secondmates retain bounded foreground checkpoints, and their private global Stop-hook registry entry delegates to `bin/fm-turnend-guard.sh` and returns its exit 2 plus stderr reason when a needed cycle is missing at turn end.
Kimi converting that result into one bounded correction turn remains a live-unverified host assumption owned by the opt-in Kimi primary guard.
...
Focus transitions normally enqueue one durable `refill` wake, with a `signal` fallback when the queue cannot accept `refill`.
A focus switch recorded by the pre-prompt adapter emits no follow-up wake because the incoming prompt already opened the turn that observes it.
This keeps a background-task completion reminder from following its own focus update with a redundant `focus: switched` turn.
...
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, an idle live Lavish source that stays quiet until its real result wakes promptly, stale-beacon attachment to a live identity-matched holder, fresh-beacon confirmation for a newly launched child, a watcher close inside the handling window that must leave the printed acknowledgement valid, and the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy.
...
`tests/fm-kimi-harness.test.sh` drives a real Kimi hook and shared guard process for a marked secondmate home with a live child and no watcher, proving that the launch-installed registry entry returns exit 2 and the shared guard reason.
`FM_KIMI_LIVE_E2E=1 tests/fm-kimi-primary-live-e2e.test.sh` is the credentialed live guard for Kimi's still-unverified host translation and retry field.
...
Claude depends on the Stop `asyncRewake` rewake, Cursor depends on its awaited stop-hook park, Grok retains native background-completion notifications, and Codex and Kimi retain bounded foreground checkpoints.
