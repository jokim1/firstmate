...
| `fm-test-run.sh`         | Behavior-test runner: selection, portable lanes, bounded concurrency, budgets, coverage and parallel shard-drift guards, timing/JSON; refuses to execute in the repository primary checkout when `FM_TASK_ID` marks a task worker |
...
| `fm-status-gc.sh`        | Retire one finished task's orphaned status record, refusing anything but that exact leak |
