...
| `fm-focus.sh`            | Owner of the durable primary-focus snapshot; its header and `--help` own the schema and mutation contract |
| `fm-focus-prompt-hook.sh`| Fail-open primary pre-prompt adapter for `fm-focus.sh`; its header owns the adapter contract |
