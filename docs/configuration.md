...
`tmux` is the verified reference backend (see [`docs/tmux-backend.md`](tmux-backend.md)); `herdr`, `zellij`, `orca`, `cmux`, and `playbot` are experimental backends (see [`docs/herdr-backend.md`](herdr-backend.md), [`docs/zellij-backend.md`](zellij-backend.md), [`docs/orca-backend.md`](orca-backend.md), and [`docs/cmux-backend.md`](cmux-backend.md); Playbot contract docs land with the additive lane as `docs/playbot-lanes.md`).
Treehouse remains the worktree provider for tmux, herdr, zellij, and cmux, since herdr, zellij, and cmux are session providers only; Orca and Playbot each provide both the task worktree and the task endpoint.
...
Zellij, Orca, and Playbot are never auto-detected and never become the home default through silent detection; select them only by explicit per-task `--backend`, a present captain instruction, `FM_BACKEND`, or local `config/backend`.
...
A Playbot task records `window=playbot:<thread-id>`, `harness=codex`, `playbot_project_id=`, `playbot_project_root_id=`, `playbot_workspace_id=`, `playbot_thread_id=`, `playbot_route_gen=`, and `playbot_delivery_id=`.
...
Playbot has no default session and is never selected by ambient markers; Windows remains unsupported for the native lane until a separate live proof.
...
FM_POLL=15              # seconds between watcher poll cycles; normalized by fm_poll_seconds (bin/fm-wake-lib.sh) for both the watcher sleep and grace - positive integers/decimals kept as canonical base-10 (zero-padded 010/0008 -> 10/8), empty/zero/negative/non-numeric/sub-resolution-round-to-0/values above FM_POLL_MAX_SECONDS fall back to 15
FM_POLL_MAX_SECONDS=86400 # hard ceiling for a accepted FM_POLL value inside fm_poll_seconds; larger or saturating inputs fall back to 15 so huge integers cannot overflow grace or break sleep
...
FM_GUARD_GRACE=         # optional override for beacon stale grace (seconds); absent defaults via fm_guard_grace_seconds to min(floor(60 * fm_poll_seconds), FM_GUARD_GRACE_MAX) so grace tracks the same normalized cadence the watcher sleeps (900 at the default 15s poll) without unbounded scaling
FM_GUARD_GRACE_MAX=3600 # hard ceiling (seconds) on poll-derived beacon stale grace; independent of FM_POLL so an absurd poll cannot suppress genuine down-detection; explicit FM_GUARD_GRACE is not capped
...
FM_ARM_CONFIRM_TIMEOUT=10   # seconds fm-watch-arm waits to confirm a newly launched watcher's fresh generation-bound beacon or attach to another live identity-matched holder before reporting FAILED; default 30 on Git Bash/MSYS
FM_ARM_ATTACH_POLL=0.5  # seconds between checks while fm-watch-arm follows an existing live identity-matched watcher cycle
...
FM_WATCHER_STALE_GRACE=   # optional; defaults to the same poll-derived grace as FM_GUARD_GRACE; seconds a direct duplicate watcher tolerates a stale live-holder beacon before erroring, and the basis for its pending-reply mid-poll beacon interval (one third of this value)
...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; it also bounds a local secondmate's wake-loop stall exemption from the unchanged-head marker's first-sighting time, because that parent-side observation has no completed-turn marker; inspection-only, never an automatic interrupt or restart; a declared external wait or verified captain-held transfer takes the FM_PAUSE_RESURFACE_SECS recheck below instead
...
FM_SECONDMATE_WAKE_STALL_SECS=300  # how long the oldest valid foreign wake-queue row must stay unmoved at the head of that queue, measured from when it reached the head rather than from when it was queued, before an endpoint-recorded local secondmate produces one durable parent wake-loop-stall notification; an exact live shared busy verdict defers that notification only until FM_BUSY_TURN_MAX_SECS from first sight, while idle, unknown, dead, and over-age busy states still alarm; zero or invalid values use this same default
