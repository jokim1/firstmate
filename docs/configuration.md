...
The same bootstrap run emits `SECONDMATE_LIVENESS:` only when a registered secondmate is skipped, its relaunch fails, or its relaunch had to proceed without the sweep's relaunch serialization; already-live and successfully relaunched secondmates are handled silently.
