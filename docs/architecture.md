...
Before executing an armed PR poll, the watcher authenticates its private single-link check and data files against the state directory's current device, their registered inode components, and their registered content hashes; the device components stored at registration are provenance rather than persistent identity because macOS device numbers can change across mounts or reboots.
...
A pool worktree is returned only after teardown passes the slot-ownership proof: a contradictory task record refuses by default, while the explicit foreign-worktree recovery may retire only a proven stale record and never returns or touches the live owner's slot.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, slot-ownership proof, PR-discovery fallback, pre-teardown run conclusion, and stale-lock recovery procedure; [`tests/fm-teardown-endpoint-safety.test.sh`](../tests/fm-teardown-endpoint-safety.test.sh), [`tests/fm-secondmate-safety.test.sh`](../tests/fm-secondmate-safety.test.sh), and [`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh) pin the default slot-collision refusal and its explicit stale-record recovery.
