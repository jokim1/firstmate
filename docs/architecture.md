...
Once that row has stayed unmoved at the head of that queue for `FM_SECONDMATE_WAKE_STALL_SECS`, measured from when it reached the head rather than from when it was queued, the primary appends one keyed `check` wake naming the mate, row sequence, and observed age unless the shared busy-state contract proves the endpoint is live and still handling a turn within `FM_BUSY_TURN_MAX_SECS` of that first sighting; keeping the original observation time means a healthy long turn stays silent while a dead, non-busy, or over-age busy loop still alarms, and parent receipts plus queued-key deduplication suppress repeats for the same row across watcher and handling crashes.
...
Otherwise it appends a durable notification and its deduplicated refill wake, records the canonical identity in that marker (replacing a different PR previously recorded for the task), and publishes a private retirement receipt bound to the poll's registration, bytes, file identities, metadata, provider, URL, and task ID.
