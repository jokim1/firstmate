...
A rejected poll-shaped artifact family gets a dedicated actionable wake that names the affected task, says merge watching stopped, and gives the `bin/fm-pr-check.sh <task-id> <pr-url>` re-arm command; generic rejected checks remain a separate wake, and a cycle containing both rejection classes enqueues both before waking once.
`tests/fm-pr-check-security.test.sh` pins the active-poll device-drift exception, exact retirement identities, malformed poll-family classification, and combined rejection delivery.
