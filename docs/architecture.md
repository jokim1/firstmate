...
Capacity-freeing lifecycle transitions enqueue one advisory `refill` wake, deduplicated by wake kind, so firstmate re-evaluates ready work against the structured fleet's free capacity without the wake selecting, recommending, or spawning work itself.
The headers of `bin/fm-wake-lib.sh`, `bin/fm-watch.sh`, and `bin/fm-teardown.sh` own the exact enqueue boundary.
...
When a canonical validated PR poll returns exactly `merged`, the watcher appends that durable notification and its deduplicated refill wake before publishing a private receipt bound to the poll's registration, bytes, file identities, metadata, provider, URL, and task ID.
...
When only an owned child's current classification is unavailable, resolution degrades per-child: the home keeps a healthy classification from live structured evidence (active work, captain decisions, holds, or no active work), names the unresolvable children in `unresolved_children`, and does not poison the whole-home verdict; every other inventory contradiction stays strict and still marks the home unknown.
...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it coordinates tracked-child startup and singleton attachment, then prints an honest `started`, `attached`, or nonzero `FAILED` status.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work, a process-event source, Relay polling, or a queued wake needs supervision and no identity-matched watcher lock with a fresh beacon is live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
...
### Follow-up queue (parked steers)

`bin/fm-send-followup-lib.sh` is the single owner of firstmate's per-task follow-up queue.
When `fm-send` targets a mid-turn task selector, ordinary steers park in `state/<id>.followup-queue/` instead of the harness busy-queue, so a later terminal `done`/`failed` cannot replay ghost instructions into a finished incarnation.
Dispatch uses an exclusive lease (foreign live owners wait so FIFO is preserved: old head before a new steer), a setup precheck, and a final terminal check at the last instant before backend transport; teardown invalidates for hygiene.
A residual race of milliseconds remains after that final check until `send-keys` completes; a terminal status published only in that window can still land.
That residual is a captain-accepted known limitation (option B, 2026-08-13); full linearization of status publication with transport is out of scope.
See the library header for the complete contract, layout, and exit codes.
