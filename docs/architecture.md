...
The primary's focus is a separate atomic snapshot: before a supported primary harness dispatches a new captain prompt, its fail-open adapter asks `bin/fm-focus.sh` to suspend any nonterminal active focus and install the new focus, preserving one durable resume owner without becoming a scheduler or second backlog.
