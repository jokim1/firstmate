...
The primary's focus is a separate atomic snapshot: before a verified tracked pre-prompt adapter dispatches a new captain prompt, it asks `bin/fm-focus.sh` to suspend any nonterminal active focus and install the new focus, preserving one durable resume owner without becoming a scheduler or second backlog.
The [README](../README.md) Restart-proof feature summary owns current harness coverage.
