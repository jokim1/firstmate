...
The primary's focus is a separate atomic snapshot: before Claude, Grok, Pi, `pi-signed`, or OpenCode dispatches a new captain prompt, its fail-open adapter asks `bin/fm-focus.sh` to suspend any nonterminal active focus and install the new focus, preserving one durable resume owner without becoming a scheduler or second backlog. Codex project prompt hooks are not reachable through the verified tracked configuration, and Kimi has no tracked primary adapter, so Phase 0 does not claim focus lifecycle coverage for either harness.
`bin/fm-focus.sh`'s header and `--help` own the snapshot schema and mutation contract, while `bin/fm-focus-prompt-hook.sh` owns adapter filtering and fail-open behavior.
