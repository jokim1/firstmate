...
A capacity-freeing status transition enqueues an advisory `refill` when newly classified, while a later turn-end or `working:` append does not re-enqueue from the stale tail.
Refills are deduplicated by wake kind so firstmate re-evaluates ready work against the structured fleet's free capacity without the wake selecting, recommending, or spawning work itself.
The headers of `bin/fm-wake-lib.sh`, `bin/fm-watch.sh`, and `bin/fm-teardown.sh` own the exact enqueue boundaries, including the accepted rare duplicate limit for status publication failures.
...
The idle predicate treats three signals as positive execution: an exact semantic busy verdict, an authoritative working run-step whose active-step record remains inside no-mistakes' configured quiet-warning window, or a verified pane process that is neither the harness nor a shell.
The source-owned run freshness check prevents a persisted `running` record from masking a dead pipeline, while process attribution prevents an unverified pane title from doing the same.
All three signals are exempt from staleness only until their last completed turn or explicit native-harness progress reaches `FM_BUSY_TURN_MAX_SECS` (`bin/fm-classify-lib.sh` owns marker selection, the shared age predicate, and the default); past that common bound the pane is routed through the same wedge escalation, with the identical reason, escalation count, worktree-write deferral, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
The same bounded predicate gates the steering-inbox re-ring ladder, so a healthy run or foreground command is not rung as idle and stale execution evidence cannot defer delivery forever.
A crew that declared an external wait (`paused:`) or a verified captain-held transfer is the one exception to the common bound: its busy verdict supplies liveness while identifying the long-running foreground call as the declared wait, so it takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation, except that a captain-held transfer is not rechecked while the away-posture record exists.
Lifting the declaration restores the unchanged execution-evidence wedge path, while a pane with no positive execution evidence returns to the existing idle declared-wait classification.
...
Before executing an armed PR poll, the watcher strictly authenticates its private single-link check and data files against their registered device-and-inode identities and content hashes; if a volume remount changed only the device number, it proves that every byte, inode, mode, link count, and live-filesystem membership still matches, then re-records the live device under the task's control and poll-publication locks before capturing strictly again.
Metadata authentication requires exactly one canonical `pr=` and validates the format of any following `pr_head=`, but otherwise accepts well-formed `key=value` lines after `pr=` so relaunch and captain-hold writers cannot disarm a watch merely by appending their keys in a different order; malformed lines still fail closed.
An ID with either poll sidecar in any filesystem shape is classified exclusively as a PR poll, so custom-check trust cannot override failed PR authentication.
A rejected poll-shaped artifact family gets a dedicated actionable wake that names the affected task, says merge watching stopped, and gives the `bin/fm-pr-check.sh <task-id> <pr-url>` re-arm command, while generic rejected checks remain a separate wake.
The watcher durably enqueues both rejection classes before beginning normal authenticated-check execution, so an actionable custom check or valid merged poll can wake without losing either rejection row; when no executed check wakes, the buffered rejection triggers one combined wake.
`tests/fm-pr-check-security.test.sh` pins metadata key-order independence, the active-poll device-drift exception, exact retirement identities, poll-family classification and custom-trust exclusion, authenticated-check execution, and durable rejection delivery.
...
After successful outcome publication, the watcher immediately delivers the emitter's local actionable poll row, enqueues one deduplicated `refill` wake on the first (non-absorbed) merged notification because capacity has freed, and publishes a private retirement receipt bound to the poll's registration, bytes, file identities, metadata, provider, URL, and task ID.
...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it coordinates tracked-child startup and singleton attachment, verifies the watcher is genuinely alive with a fresh liveness beacon, and prints an honest `started`, `attached`, or nonzero `FAILED` status.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-wait vocabulary (a `paused:` external wait and a verified `captain-held` transfer alike, through one combined predicate), the live-execution idle predicate and its busy-turn ceiling, and status-scan primitives.
...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while herdr starts with an atomic `pane run`, then confirms through native agent state on idle baselines, a composer empty fallback when native stays idle, or a pre-submit rendered-footer transition when that baseline is unavailable.
...
Its local-only typed plane - harness-native invocations and explicit backend targets - selects a backend-specific first-submit popup settle for slash commands and for codex `$...` skill invocations using metadata-routed target `harness=` values, then adds its own `FM_SEND_SETTLE` pause after successful typed sends so immediate peeks catch the receiving turn starting; the sub-supervisor uses only the shared submit core and does not pay that post-submit pause.
...
For pipeline-rewritten or cherry-picked work, teardown can additionally bind a live merged PR's final head to current or recorded task metadata, or compare every task-changed path with the live default under a caller-supplied exception-glob allowlist.
That allowlist cannot constitute landing proof by itself, recorded PRs must still read live as merged before either exceptional proof can authorize cleanup, and every non-allowlisted differing path is printed and refused.
