...
For pipeline-rewritten or cherry-picked work, teardown can additionally bind a live merged PR's final head to current or recorded task metadata, or compare every task-changed path with the live default while excluding only caller-supplied regenerated-path globs.
Recorded PRs must still read live as merged before either exceptional content proof can authorize cleanup, and every non-allowlisted differing path is printed and refused.
