...
In Claude mode, an unread record in `state/.wake-queue` also holds the Stop-owned cycle even when no task, process-event source, or Relay poll remains.
