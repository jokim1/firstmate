...
The compatibility seed in `bin/fm-playbot-lanes.mjs` is the authoritative per-release record of read-only schema, IPC string, and wire-contract facts for proven releases (currently `0.90.0`, `0.92.0`, `0.93.1`, `0.94.0`, `0.101.0`, `0.104.0`, and `0.106.0`).
Playbot `0.94.0` removed the standalone `workspace:create` and `threads:openThread` channels, and `0.101.0`, `0.104.0`, and `0.106.0` retain that contract, so each seed entry also fixes the release's thread-open and workspace-create wire contracts: the abstract operation names and evidence keys stay stable, while on those releases both operations go over `threads:launch` (workspace creation is fused with opening the workspace's first thread, and thread ids are minted by the app) and each evidence record annotates the real wire channel.
...
On a release with the fused contract (`0.94.0`, `0.101.0`, `0.104.0`, and `0.106.0`), the create step already opens the workspace's first thread, so the smoke adopts that thread instead of opening a second one and still records both the `workspace:create` and `threads:openThread` evidence keys from the single launch.
