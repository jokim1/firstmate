...
Playbot also owns app-written Godot plugin churn during teardown, so `bin/fm-teardown.sh` ignores uncommitted changes confined to `addons/playbot/**`, `project.godot`, and `.fm/**` only for backend=playbot tasks.
