...
After the new workspace converges to one exact task endpoint bound to one exact parent workspace id, the journal advances to a version 2 binding that records the physical home, named session, endpoint, parent, and immutable expected labels.
...
A same-identity version 2 binding may replace one exact agent-free restart husk in place only when the physical home, session, metadata endpoint, unique token match, workspace shape and labels, recorded parent workspace identity and label, and non-target focus snapshot all agree.
...
Herdr's `pane send-text` and a later `pane send-keys enter` are not a reliable submission pair for an idle agent pane.
Both calls can succeed while the composer keeps the text across later turn boundaries.
Every Firstmate line submission therefore uses Herdr's bracketed-paste-aware `pane run` as one atomic text-plus-submit request, matching the already-working spawn-time command path.
Ordinary local text steers use the durable steering inbox and submit only its best-effort constant doorbell through this same core.
Enter, Escape, and Ctrl-C remain supported as individual keys.
Typed-plane text is sent once in the atomic first attempt; only Enter is retried when confirmation finds a completion popup or pending composer.
The shared harness-aware settle applies after Herdr's first atomic attempt so the UI can render before confirmation.

The adapter captures its native and rendered baselines before the atomic first attempt.
...
On an already active or unreadable baseline, the adapter falls back to conservative composer clearance, with a pre-submit rendered-footer transition when that baseline is unavailable.
...
That fallback alone reported every delivered steer as unconfirmed, so it is paired with a rendered-footer transition: the pane's verified busy footer is read once before the first submission, and an idle-to-busy transition across that submission confirms delivery.
...
`bin/fm-afk-launch.sh` creates a dedicated unfocused Herdr workspace for every harness that runs the away daemon, runs the daemon there with an explicit supervisor target and backend, records the exact daemon pane, and closes only that pane on stop.
Pi and pi-signed no longer launch the away daemon; their ordinary supervision session continues under the posture record.
Keeping the daemon outside the captain pane prevents its own long-lived process from making the injection target classify as busy.
It never splits the captain's active tab and never uses shell `&`.
Recovery reconciles only the recorded exact id.
The rendered busy fallback bounds classification to the bottom physical footer, tolerates blank padding inside it, and ignores older rows above that boundary.
