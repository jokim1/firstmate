...
- repeat reconciliation journals one unchanged command request only once without answering it; asset generation accepts an in-root linked file but leaves out-of-root `file:` and remote URL references pending with the blocked reference named.
