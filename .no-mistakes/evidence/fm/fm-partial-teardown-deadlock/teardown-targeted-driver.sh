...
. "/Users/josephkim/.no-mistakes/worktrees/9ffe04aa3c01/01M2G885SMMZPMPA47P8PZZRN8/tests/lib.sh"
...
test_teardown_enqueues_refill_wake() {
  local case_dir drain_out drain_err refill_n leftover sequence generation
  ...
  FM_STATE_OVERRIDE="$case_dir/state" "$ROOT/bin/fm-wake-drain.sh" > "$drain_out" 2> "$drain_err" \
  ...
  [ -n "$sequence" ] && [ -n "$generation" ] \
    || fail "teardown-refill: drain omitted its acknowledgement boundary"
  FM_STATE_OVERRIDE="$case_dir/state" "$ROOT/bin/fm-wake-drain.sh" \
    --ack-through "$sequence" --recovery-generation "$generation" \
    || fail "teardown-refill: refill could not be acknowledged"
